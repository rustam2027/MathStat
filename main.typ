...
#question(16)[
  #definition([P-value])[в критериях согласия вероятность принять статистике критерия такое или большее значение. 
  $epsilon^* = 1 - F_eta (rho(overline(X))) = P(eta > rho ( overline(X)) ); rho(overline(X))$ -- реализация статистики.

  Если p-value меньше заданного уровня значимости, то нулевая гипотеза отвергается в пользу альтернативной. В противном случае она отвергается.] 
]
